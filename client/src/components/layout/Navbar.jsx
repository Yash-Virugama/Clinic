...
const Navbar = () => {
  ...
  const getPanelLinks = (userVal, isMobileVal = false) => {
    const clickHandler = isMobileVal ? () => setMobileMenuOpen(false) : undefined;
    const btnClass = isMobileVal
      ? "w-full text-center py-3 text-sm font-bold uppercase tracking-wider rounded-xl bg-slate-100 text-secondary"
      : "px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-xl bg-slate-100 hover:bg-slate-200 text-secondary transition-premium";
    ...
    if (userVal.role === "admin") {
      return (
        <>
          <NavLink to="/clinic" onClick={clickHandler} className={btnClass}>
            Clinic Panel
          </NavLink>
          <NavLink to="/admin" onClick={clickHandler} className={btnClass}>
            Admin Panel
          </NavLink>
        </>
      );
    ...
    if (["assistant", "intern", "physiotherapist", "receptionist"].includes(userVal.role)) {
      const hasClinicAccess = userVal.permissions && userVal.permissions.includes("clinic:dashboard");
      return (
        <>
          {hasClinicAccess && (
            <NavLink to={`/staff/${userVal.role}/clinic`} onClick={clickHandler} className={btnClass}>
              Clinic Panel
            </NavLink>
          )}
          <NavLink to={`/staff/${userVal.role}`} onClick={clickHandler} className={btnClass}>
            Staff Panel
          </NavLink>
        </>
      );
    ...
    // Patient
    return (
      <NavLink to="/dashboard" onClick={clickHandler} className={btnClass}>
        Dashboard
      </NavLink>
    );
  };
