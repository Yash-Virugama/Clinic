    ...
    <div className={`flex flex-col items-center justify-center ${fullPage ? 'h-screen w-screen fixed inset-0 bg-slate-50 z-50' : 'h-full w-full py-12'}`}>
      <div className="relative flex flex-col items-center gap-6 animate-pulse-slow">
        ...
        <div className="relative">
          ...
          <svg className="w-16 h-16 relative animate-float" style={{ color: 'var(--primary)' }} viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Shoulder line */}
            ...
            <circle cx="18" cy="11.5" r="2.5" fill="var(--accent)" />
            <circle cx="18" cy="27" r="2.5" fill="var(--accent)" />
          </svg>
        </div>
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="w-48 h-1.5 bg-slate-200/80 rounded-full overflow-hidden relative">
            <div className="h-full absolute top-0 left-0 rounded-full animate-loading-bar" style={{ backgroundImage: 'linear-gradient(to right, var(--primary), var(--accent))', width: '40%' }} />
          </div>
