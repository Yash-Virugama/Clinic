...
const ClinicTomorrowVisits = () => {
  ...
  const fetchTomorrowAppointments = async () => {
    try {
      ...
      console.error(err);
      toast.error("Failed to load tomorrow's appointments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      ...
      await fetchTomorrowAppointments();
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6 text-left animate-page-entrance">
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto py-1">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search tomorrow's sessions..."
            value={searchQuery}
      ...
      {filteredAppointments.length === 0 ? (
        <div className="bg-white border border-slate-200/60 rounded-[32px] p-16 shadow-sm text-center">
          ...
          <h3 className="text-base font-bold text-secondary mb-1">No Sessions Found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto text-center">
            {searchQuery ? "No sessions match your search query." : "There are no patient sessions logged or scheduled for tomorrow's date yet."}
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full animate-fade-in">
            {paginatedAppointments.map((appointment) => (
              <AppointmentCard
                ...
                patientCode={appointment.patient?._id ? getPatientCode(appointment.patient._id).toLowerCase() : ""}
...
export default ClinicTomorrowVisits;
