...
const AdminDashboard = () => {
  ...
  const rolePrefix = user?.role === "admin" ? "/admin" : `/staff/${user?.role}`;
  ...
  const rawCards = [
    {
      to: `${rolePrefix}/services`,
      title: "Services Count",
      count: stats?.totalServices || 0,
      label: "View Services",
      visible: hasPerm(["services:view", "services:manage"]),
      icon: (
    ...
    },
    {
      ...
      label: "Manage Posts",
      visible: hasPerm(["blogs:view", "blogs:manage"]),
      icon: (
        <svg className="w-6 h-6 text-primary stroke-[2]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      to: `${rolePrefix}/resources`,
      title: "Shared Resources",
      count: stats?.totalResources || 0,
      label: "Vault Manager",
      visible: hasPerm(["resources:view", "resources:manage"]),
      icon: (
        <svg className="w-6 h-6 text-primary stroke-[2]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      isPending: (stats?.pendingTestimonials || 0) > 0,
      visible: hasPerm(["testimonials:view", "testimonials:manage"]),
      icon: (
        <svg className={`w-6 h-6 stroke-[2] ${(stats?.pendingTestimonials || 0) > 0 ? "text-amber-500 animate-pulse" : "text-primary"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
        </svg>
      )
    },
    {
      to: `${rolePrefix}/contacts`,
      title: "Contact Messages",
      count: stats?.totalContacts || 0,
      label: "View Inbox",
      visible: hasPerm(["contacts:view", "contacts:manage"]),
      icon: (
        <svg className="w-6 h-6 text-primary stroke-[2]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  const cardsData = rawCards.filter((card) => card.visible);
            ...
            {hasPerm(["reports:view"]) && (
              ...
              >
                ...
                <div className="text-left">
                  <span className="text-xs font-bold text-secondary block">Download Patients Report</span>
                  <span className="text-[10px] text-slate-400 block mt-0.5">Get in detail patient report</span>
                </div>
              </button>
            )}

            {hasPerm(["blogs:manage"]) && (
              <Link
                to={`${rolePrefix}/blogs`}
