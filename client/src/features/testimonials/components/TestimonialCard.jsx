...
const TestimonialCard = ({ testimonial, truncate = false }) => {
  const rating = testimonial.rating || 5;
  const hasImage = !!testimonial.user?.image;
  const initials = testimonial.patientName
    ? testimonial.patientName.split(" ").map(n => n.charAt(0)).join("").toUpperCase().slice(0, 2)
    : "P";

  return (
    <article 
      className="testimonial-card glass-card p-8 rounded-3xl border border-slate-200/60 transition-premium hover:-translate-y-1.5 hover:shadow-xl hover:border-primary/20 relative flex flex-col justify-between overflow-hidden min-h-[320px] w-full text-left"
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <p className="text-sm sm:text-base text-secondary font-body italic leading-relaxed mb-8 relative pr-14">
          {truncate && testimonial.content.length > 50 ? (
            <>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-200/60 relative z-10">
        <div className="flex items-center gap-4">
          {/* Rounded avatar or initials */}
          {hasImage ? (
            <img 
              src={testimonial.user.image} 
              alt={testimonial.patientName || "Patient"} 
              className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm shrink-0"
            />
          ) : (
            <div className="w-12 h-12 rounded-full bg-primary/10 border-2 border-primary/20 flex items-center justify-center font-bold text-sm text-primary shadow-inner shrink-0">
              {initials}
            </div>
          )}
          <div>
            <h4 className="font-bold text-sm text-secondary font-heading leading-tight">{testimonial.patientName || "Anonymous Patient"}</h4>
